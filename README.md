...
Before changing anything, `tools` checks all eight command names in `~/.local/bin`, `~/bin`, and `/usr/local/bin`. It installs and verifies the Ubuntu packages first. It then permanently removes exact matching files or symlinks from `~/.local/bin`, plus matching `.pre-linux-setup-apt-*` files or symlinks left by the older backup policy. Directories are refused. Matches in `~/bin` or `/usr/local/bin` are renamed with a `.pre-linux-setup-apt-<timestamp>` suffix. Finally, it resolves every command to its canonical `/usr/bin` or `/usr/sbin` APT path.
