...
Ubuntu-supported machine branding sets the pretty Device Name to **MrDemonWolf, Inc.** with `hostnamectl` while preserving the static network hostname. It also installs the reviewed transparent logo for GDM through GNOME's documented `org.gnome.login-screen.logo` dconf key; the logo appears after the next sign-out or reboot. Ubuntu does not provide a supported override for Apple's firmware/SMBIOS manufacturer, so Settings may still truthfully identify the hardware as Apple and the operating system as Ubuntu. Setup never edits `/etc/os-release`, DMI data, firmware resources, or Ubuntu package-owned artwork. See [Ubuntu's Device Name guide](https://help.ubuntu.com/stable/ubuntu-help/about-hostname.html.en) and [GNOME's GDM logo guide](https://help.gnome.org/system-admin-guide/login-logo.html).
