...
- Keep the dedicated `sources` action free of APT refreshes and package installs. Bootstrap must invoke it before `apt-get update` when both known Claude source paths exist, so the guarded migration can resolve `Signed-By` conflicts before APT parses them.
