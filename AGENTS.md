...
- Keep the selected CLI utilities in the explicit `tools` action and install all of them from Ubuntu APT. After APT succeeds, move matching command shadows from `~/.local/bin`, `~/bin`, and `/usr/local/bin` to recoverable timestamped backups, then verify each command resolves to its canonical APT path. Do not silently uninstall packages owned by another package manager. Twitch CLI is unavailable in Ubuntu 26.04, so do not install it automatically or add a non-APT exception. Never configure AWS, rclone, scans, mirrors, credentials, or tokens.
