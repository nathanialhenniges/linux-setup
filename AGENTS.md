...
- Keep the systemd pretty hostname at `Nathanials Air` while the static network hostname remains unchanged. Keep MrDemonWolf, Inc. branding on Ubuntu-supported visual surfaces: GDM may use the checksum-pinned logo through its documented dconf profile/key, and the separate boot action may use the reviewed Plymouth theme. Refuse unknown owned key content. Never run the OEM first-boot workflow on this configured workstation, spoof Apple DMI/SMBIOS data, rename Ubuntu through `os-release`, replace package-owned distribution artwork, or restart GDM during setup.
