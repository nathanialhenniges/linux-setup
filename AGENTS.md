...
- Keep machine branding on Ubuntu-supported surfaces: the systemd pretty hostname may be `MrDemonWolf, Inc.` while the static network hostname remains unchanged, and GDM may use the checksum-pinned logo through its documented dconf profile/key. Refuse unknown owned key content. Never spoof Apple DMI/SMBIOS data, rename Ubuntu through `os-release`, replace package-owned distribution artwork, or restart GDM during setup.
